...
func (p *PostgresDB) BuildHNSWIndex(ctx context.Context) error {
	// Check if embeddings table has any rows
	var count int
	err := p.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM embeddings").Scan(&count)
	if err != nil {
...
func (p *PostgresDB) RebuildHNSWIndex(ctx context.Context) error {
	// Drop existing index (if exists)
	_, err := p.db.ExecContext(ctx, "DROP INDEX IF EXISTS idx_embeddings_hnsw")
	if err != nil {
...
func (p *PostgresDB) SearchANN(ctx context.Context, query []float32, k int) ([]SearchResult, error) {
	return p.SearchANNWithFilter(ctx, query, k, "")
}
...
func (p *PostgresDB) SearchANNWithFilter(ctx context.Context, query []float32, k int, contentType string) ([]SearchResult, error) {
	...
	if contentType != "" {
		...
		rows, err = p.db.QueryContext(ctx, query_sql, queryVec, contentType, candidateLimit)
	} else {
		...
		rows, err = p.db.QueryContext(ctx, query_sql, queryVec, k)
	}
	...
	results, err := p.scanSearchResults(rows)
	if err != nil {
		return nil, err
	}
...
func (p *PostgresDB) scanSearchResults(rows *sql.Rows) ([]SearchResult, error) {
